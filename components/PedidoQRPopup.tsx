        ...
        <div className="space-y-1 mb-3">
          {pedido.items.slice(0, 3).map((item, i) => (
            <p key={i} className="text-xs text-gray-300">
              <span className="font-semibold">{item.cantidad}x</span> {item.nombre}
              {item.observacion && <span className="text-amber-400 italic"> &mdash; {item.observacion}</span>}
            </p>
          ))}
          {pedido.items.length > 3 && (
            <p className="text-xs text-gray-500">+{pedido.items.length - 3} mas...</p>
          )}
        </div>
