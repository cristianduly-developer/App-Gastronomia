...
export function BottomNav() {
  ...
  return (
    <>
      {/* Banner update — mobile */}
      {updateReady && (
        <div className="fixed bottom-16 left-0 right-0 z-40 md:hidden">
          {!actualizando ? (
            <div className="mx-3 mb-1 bg-gradient-to-r from-blue-600 to-indigo-600 rounded-xl px-4 py-2.5 flex items-center justify-between gap-2 shadow-lg">
              <div className="flex items-center gap-2">
                <span className="text-base">🆕</span>
                <span className="text-xs font-semibold text-white">Nueva versión disponible</span>
              </div>
              <button
                onClick={() => { setActualizando(true); aplicarUpdate() }}
                className="bg-white text-blue-700 px-3 py-1 rounded-lg font-bold text-xs active:scale-95 transition-all"
              >
                Actualizar
              </button>
            </div>
          ) : (
            <div className="mx-3 mb-1 bg-gradient-to-r from-blue-600 to-indigo-600 rounded-xl px-4 py-2.5 flex items-center justify-center gap-2 shadow-lg">
