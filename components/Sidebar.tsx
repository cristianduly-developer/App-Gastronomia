    ...
    <aside className="hidden md:flex w-64 bg-gray-900 border-r border-gray-800 flex-col h-screen sticky top-0">
      ...
      {updateReady && (
        <div className="mx-3 mb-2 rounded-xl overflow-hidden">
          {!actualizando ? (
            <div className="bg-gradient-to-r from-blue-600 to-indigo-600 px-3 py-2.5 flex items-center justify-between gap-2">
              <div className="flex items-center gap-2 min-w-0">
                <span className="text-base flex-shrink-0">🆕</span>
                <span className="text-xs font-semibold text-white leading-tight">Nueva versión disponible</span>
              </div>
              <button
                onClick={() => { setActualizando(true); aplicarUpdate() }}
                className="bg-white text-blue-700 px-2.5 py-1 rounded-lg font-bold text-xs flex-shrink-0 active:scale-95 transition-all"
              >
                Actualizar
              </button>
            </div>
          ) : (
            <div className="bg-gradient-to-r from-blue-600 to-indigo-600 px-3 py-2.5 flex items-center justify-center gap-2">
              <span className="w-3.5 h-3.5 border-2 border-white border-t-transparent rounded-full animate-spin" />
              <span className="text-xs font-semibold text-white">Actualizando...</span>
            </div>
          )}
        </div>
      )}

      {/* Ayuda + Soporte + Logout */}
      <div className="p-3 border-t border-gray-800 space-y-1">
        ...
        <button
          onClick={async () => {
            ...
            const txt = `Hola, soy usuario de App Gastronomía y necesito soporte.\nMi mail: ${mail}\nMi problema es: `
